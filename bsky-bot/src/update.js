        ...
        switch(dischargeInfo.type) {
            case "start": {
                await connection.execute(
                    `INSERT INTO ${table} (asset_id, event_start, first_seen_at, notified_start) VALUES (?, ?, ?, 1)`,
            ...
            case "24h": {
                await connection.execute(
                    `UPDATE ${table} SET notified_24h = 1 WHERE asset_id = ? AND event_start = ?`,
            ...
            case "ended": {
                await connection.execute(
                    `UPDATE ${table} SET notified_end = 1 WHERE asset_id = ? AND event_start = ?`,
                    [discharge.asset_id, discharge.latest_event_start]
                )
                break;
            }
